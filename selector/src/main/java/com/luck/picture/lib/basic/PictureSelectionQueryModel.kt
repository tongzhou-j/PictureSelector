...
class PictureSelectionQueryModel(private val selector: PictureSelector, selectMimeType: Int) {
     ...
     * @return
     */
    fun isHeic(isHeic: Boolean): PictureSelectionQueryModel {
    ...
    fun buildMediaLoader(): IBridgeMediaLoader {
        ...
        return if (selectionConfig.isPageStrategy)
            LocalMediaPageLoader(activity, selectionConfig)
        else
            LocalMediaLoader(activity, selectionConfig)
    }
    ...
    fun obtainAlbumData(call: OnQueryDataSourceListener<LocalMediaFolder?>) {
        ...
        val loader = if (selectionConfig.isPageStrategy)
            LocalMediaPageLoader(activity, selectionConfig)
        else
            LocalMediaLoader(activity, selectionConfig)
                ...
                if (result != null && result.size > 0) {
                    ...
                    if (all != null) {
                        if (selectionConfig.isPageStrategy) {
                            (loader as LocalMediaPageLoader).loadPageMediaData(
                                all.bucketId, 1, selectionConfig.pageSize,
