...
class SlideSelectTouchListener : OnItemTouchListener {
    ...
    private var mAutoScrollDistance =
        (Resources.getSystem().getDisplayMetrics().density * 56).toInt()
    ...
    private fun notifySelectRangeChange() {
        ...
        val newStart: Int
        val newEnd: Int
        newStart = min(mStart.toDouble(), mEnd.toDouble()).toInt()
        newEnd = max(mStart.toDouble(), mEnd.toDouble()).toInt()
        if (newStart < 0) {
            return
        }
    ...
    private fun reset() {
        setActive(false)
        if (mSelectListener != null && mSelectListener is OnAdvancedSlideSelectListener) {
            (mSelectListener as OnAdvancedSlideSelectListener).onSelectionFinished(mEnd)
        }
    ...
    private fun scrollBy(distance: Int) {
        ...
        if (distance > 0) scrollDistance =
            min(distance.toDouble(), mMaxScrollDistance.toDouble()).toInt()
        else scrollDistance = max(distance.toDouble(), -mMaxScrollDistance.toDouble()).toInt()
