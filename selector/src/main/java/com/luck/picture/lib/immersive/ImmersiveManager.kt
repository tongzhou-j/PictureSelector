...
object ImmersiveManager {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                ...
                if (isDarkStatusBarIcon) {
                    ImmersiveManager.initBarBelowLOLLIPOP(baseActivity)
                } else {
                    window.setFlags(
    ...
    fun translucentStatusBar(activity: Activity, isDarkStatusBarBlack: Boolean) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (isDarkStatusBarBlack) {
                decor.systemUiVisibility = (View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR or View.SYSTEM_UI_FLAG_LAYOUT_STABLE or View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN)
            } else {
    ...
    private fun setupStatusBarView(activity: Activity) {
        val mWindow = activity.window
        var statusBarView =
            mWindow.decorView.findViewWithTag<View?>(TAG_FAKE_STATUS_BAR_VIEW)
    ...
    private fun setupNavBarView(activity: Activity) {
        ...
        var navigationBarView =
            window.decorView.findViewWithTag<View?>(TAG_NAVIGATION_BAR_VIEW)
