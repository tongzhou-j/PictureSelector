...
internal class CustomGestureDetector(context: Context, listener: OnGestureListener) {
    private var mActivePointerId: Int = CustomGestureDetector.Companion.INVALID_POINTER_ID
    ...
    private fun processTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (action and MotionEvent.ACTION_MASK) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (!this.isDragging) {
                    ...
                    // touch slop
                    this.isDragging = sqrt(((dx * dx) + (dy * dy)).toDouble()) >= mTouchSlop.toDouble()
                }
            ...
            MotionEvent.ACTION_CANCEL -> {
                mActivePointerId = CustomGestureDetector.Companion.INVALID_POINTER_ID
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (this.isDragging) {
                    if (null != mVelocityTracker) {
                        ...
                        val vX = mVelocityTracker!!.getXVelocity(mActivePointerId)
                        val vY = mVelocityTracker!!
                            .getYVelocity(mActivePointerId)

                        // If the velocity is greater than minVelocity, call
                        // listener
                        if (max(abs(vX.toDouble()), abs(vY.toDouble())) >= mMinimumVelocity.toDouble()) {
                            mListener.onFling(
        ...
        mActivePointerIndex = ev
            .findPointerIndex(
                if (mActivePointerId != CustomGestureDetector.Companion.INVALID_POINTER_ID)
                    mActivePointerId
                else
                    0
            )
