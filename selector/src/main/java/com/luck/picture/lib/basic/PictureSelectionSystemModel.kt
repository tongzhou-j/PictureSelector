...
class PictureSelectionSystemModel(private val selector: PictureSelector, chooseMode: Int) {
     ...
     */
    fun isNewKeyBackMode(isNewKeyBackMode: Boolean): PictureSelectionSystemModel {
    ...
    fun forSystemResult(call: OnResultCallbackListener<LocalMedia?>) {
        if (!DoubleUtils.isFastDoubleClick) {
            ...
            val fragment =
                fragmentManager.findFragmentByTag(PictureSelectorSystemFragment.Companion.TAG)
            if (fragment != null) {
                fragmentManager.beginTransaction().remove(fragment).commitAllowingStateLoss()
            }
            FragmentInjectManager.injectSystemRoomFragment(
                fragmentManager,
                PictureSelectorSystemFragment.Companion.TAG,
                PictureSelectorSystemFragment.Companion.newInstance() as Fragment
            )
        }
    }
