 ...
 */
open class PicturePreviewAdapter @JvmOverloads constructor(
    private val selectorConfig: SelectorConfig? = SelectorProviders.Companion.instance
        ?.selectorConfig
) : RecyclerView.Adapter<BasePreviewHolder?>() {
    private var mData: MutableList<LocalMedia?>? = null
    private var onPreviewEventListener: OnPreviewEventListener? = null
    ...
    override fun onBindViewHolder(holder: BasePreviewHolder, position: Int) {
        ...
        holder.bindData(media ?: return, position)
    }
    ...
    override fun getItemViewType(position: Int): Int {
        if (PictureMimeType.isHasVideo(mData!!.get(position)!!.mimeType)) {
            return BasePreviewHolder.Companion.ADAPTER_TYPE_VIDEO
        } else if (PictureMimeType.isHasAudio(mData!!.get(position)!!.mimeType)) {
            return BasePreviewHolder.Companion.ADAPTER_TYPE_AUDIO
        } else {
            return BasePreviewHolder.Companion.ADAPTER_TYPE_IMAGE
        }
    }
    ...
    fun destroy() {
        for (key in mHolderCache.keys) {
            val holder = mHolderCache.get(key)
            holder?.release()
        }
    }
}
