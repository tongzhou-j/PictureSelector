...
object CameraUtils {
    ...
    fun buildImageContentValues(cameraFileName: String?, mimeType: String?): ContentValues {
        ...
        if (cameraFileName.isNullOrEmpty()) {
            values.put(MediaStore.Images.Media.DISPLAY_NAME, DateUtils.getCreateFileName("IMG_"))
        } else {
            val lastDotIndex = cameraFileName.lastIndexOf(".")
            if (lastDotIndex == -1) {
                values.put(MediaStore.Images.Media.DISPLAY_NAME, DateUtils.getCreateFileName("IMG_"))
            } else {
                val suffix = cameraFileName.substring(lastDotIndex)
        ...
        values.put(
            ...
            if (mimeType.isNullOrEmpty() || mimeType.startsWith(MIME_TYPE_PREFIX_VIDEO)) MIME_TYPE_IMAGE else mimeType
        )
    ...
    fun buildVideoContentValues(cameraFileName: String?, mimeType: String?): ContentValues {
        ...
        values.put(
            ...
            if (mimeType.isNullOrEmpty() || mimeType.startsWith(MIME_TYPE_PREFIX_IMAGE)) MIME_TYPE_VIDEO else mimeType
        )
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
